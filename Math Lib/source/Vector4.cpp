...
Vector4 Vector4::operator*(const real scalar)
{
	return Vector4(x * scalar, y * scalar, z * scalar, w * scalar);
}

real Vector4::dot(Vector4 & other) const
{
	real Xdot = x * other.x;
	real Ydot = y * other.y;
	real Zdot = z * other.z;
...
real Vector4::magnitude()
{
	...
	real powW = (real)pow(w, 2);
	real magnitude = (real)sqrt(powX + powY + powZ + powW);
...
real &Vector4::operator[] (const int &index)
{
	return V[index];
}

const real  Vector4::operator[] (const int index) const
{
	return V[index];
}
